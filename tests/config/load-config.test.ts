...
describe("loadConfig", () => {
  it("parses a valid configuration into a typed config", () => {
    const config = loadConfig(validYaml, { expectedRepository: "acme/mergepay-demo" });
    expect(config.version).toBe(1);
    expect(config.repository).toBe("acme/mergepay-demo");
    ...
    expect(config.payout.amounts["mergepay-5"]).toBe("5");
  ...
  it("rejects an unsupported schema version", () => {
    ...
    expect(() => loadConfig(bad, { expectedRepository: "acme/mergepay-demo" })).toThrow(/version/i);
  });
...
describe("loadConfig type shape", () => {
  it("returns a value assignable to MergePayConfig", () => {
    const config: MergePayConfig = loadConfig(validYaml, {
      expectedRepository: "acme/mergepay-demo",
    });
