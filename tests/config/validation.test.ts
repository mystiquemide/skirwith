...
describe("config semantic validation", () => {
  it("rejects an invalid token address", () => {
    ...
    expect(() => loadConfig(yaml, { expectedRepository: "acme/skirwith-demo" })).toThrow(
      /address/i,
    );
  });
  ...
  it("rejects a negative amount", () => {
    const yaml = base.replace('skirwith-5: "5"', 'skirwith-5: "-5"');
    expect(() => loadConfig(yaml, { expectedRepository: "acme/skirwith-demo" })).toThrow(/amount/i);
  });

  it("rejects an amount above the configured maximum", () => {
    const yaml = base.replace('skirwith-5: "5"', 'skirwith-5: "30"');
    expect(() => loadConfig(yaml, { expectedRepository: "acme/skirwith-demo" })).toThrow(
      /cap|max/i,
    );
  });
  ...
  it("accepts an amount exactly equal to the configured maximum", () => {
    const yaml = base.replace('skirwith-5: "5"', 'skirwith-25: "25"');
    expect(() => loadConfig(yaml, { expectedRepository: "acme/skirwith-demo" })).not.toThrow();
  });

  it("rejects an amount with fractional precision beyond the token decimals", () => {
    const yaml = base.replace('skirwith-5: "5"', 'skirwith-5: "0.0000001"');
    expect(() => loadConfig(yaml, { expectedRepository: "acme/skirwith-demo" })).toThrow(
      /fraction|decimal/i,
    );
  });
