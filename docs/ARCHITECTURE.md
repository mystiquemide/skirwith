...
The GitHub receipt contains readable Markdown plus a hidden versioned marker encoding only the minimum safe duplicate fields: product/version, payment key, request hash, execution ID, status, repository, PR, and merge SHA. The marker is authenticated with an HMAC-SHA256 tag computed over the marker payload using a dedicated versioned receipt-signing secret (independent of the KeeperHub broadcast key), recorded with an explicit key id. The comment-backed receipt store verifies the tag against the active and (during rotation) previous signing keys and fails closed on any missing, tampered, unknown-key, or differently-signed marker, so attacker-authored comments cannot fabricate execution state or suppress payouts. The write path updates only a marker the action can authenticate as its own; forged squatters are never edited and a fresh action-owned receipt comment is created instead.

The settlement orchestrator composes the pure policy, canonical identity, and provider layers into one deterministic flow: evaluate policy; on approval derive the payment key and request hash; resolve any existing receipt (confirmed same-hash is a duplicate, pending resumes polling the original execution, changed hash is a conflict, prior failure is manual review); otherwise write a durable pending reservation, simulate, broadcast the exact parameters with the idempotency key only after the reservation persists, record the submitted execution, poll to a terminal state, and update the receipt. A blocked or reverted path performs no provider call; a failed reservation means no broadcast; a lost broadcast response, expired poll deadline, or receipt-save failure produces manual review that preserves the execution id, and the durable reservation ensures a later run resolves to manual review or resume-poll and never rebroadcasts.
